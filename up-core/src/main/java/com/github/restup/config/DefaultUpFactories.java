...
enum DefaultUpFactories implements UpFactories {
    ...
    static <T> List<T> getInstances(ConfigurationContext configurationContext,
        Map<String, List<String>> resources,
        Class<T> clazz)
        throws ClassNotFoundException, IllegalAccessException, InstantiationException {
        List<T> result = new ArrayList<>();
        String key = clazz.getName();
        if (isEnabled(key, configurationContext)) {
            List<String> classes = resources.get(key);
            if (classes != null) {
                for (String s : classes) {
                    if (isEnabled(s, configurationContext)) {
    ...
    static boolean isEnabled(String s, ConfigurationContext configurationContext) {
        boolean enabled = isTrue(configurationContext.getProperty(s + ".enabled"), true);
        boolean disabled = isTrue(configurationContext.getProperty(s + ".disabled"), false);
    ...
    @Override
    public <T> List<T> getInstances(ConfigurationContext configurationContext, Class<T> clazz) {
        try {
            return getInstances(configurationContext, resources, clazz);
        } catch (Exception e) {
